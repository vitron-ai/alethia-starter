...
| File | Purpose |
|---|---|
| `smoke.alethia` | Sign-in screen renders correctly. The cheapest check. |
| `signin-flow.alethia` | Happy path — sign in, land on dashboard, nav is present. |
| `crud-flow.alethia` | Task list exercise — add a task, assert it appears. |
| `edit-flow.alethia` | Edit-task modal — click Edit → modal opens → update title → Save → verify in list. Tests conditional DOM + pre-filled input replacement. |
| `search-flow.alethia` | Search filter — type into search box, verify filtered-in tasks appear and filtered-out ones go absent. Exercises `ASSERT_EXISTS` and `assert X is not visible` against a live-filtered list. |
| `priority-flow.alethia` | Priority dropdown — change a task's priority via `<select>`. Exercises the SELECT command + the change-toast pattern. |
| `toast-flow.alethia` | Toast notifications — add task → assert toast appears → wait out the auto-dismiss → assert it's gone. Tests transient UI with `assert X is not visible`. |
| `count-flow.alethia` | Text-count assertions — asserts exact count strings ("3 total", "2 active"), then verifies the counts change after adding a task. |
...
The agent will call `alethia_tell` once per file. For `safety.alethia`, the `expect block:` step **passes** if the EA1 gate blocks the action, **fails** if the gate lets it through. That's the verifiable-safety primitive.
...
- **Keep files focused** — one `.alethia` file = one scenario. Parallel runs are easier to scan when tests aren't interleaved.
...
A GitHub Actions workflow that installs Alethia via npm, serves this app, and runs each `.alethia` file in sequence is straightforward but not included in starter v0. Once added, this is where the README badge earns its green check.
